...
emuMethods.verifyEmulatorsAreAvailable = async function (udid) {
...
emuMethods.fingerprint = async function (fingerprintId, udid = undefined) {
  ...
  await this.verifyEmulatorsAreAvailable();
  if (udid) {
...
emuMethods.mockLocation = async function (geo, udid = undefined) {
  if (_.isUndefined(geo)) {
    log.errorAndThrow('Geo location parameter must be defined');
  }
  await this.verifyEmulatorsAreAvailable();
  if (udid) {
    await this.setDeviceId(udid);
  }
  let commandArgs = ['emu', 'geo', 'fix', _.ceil(geo.latitude, 5), _.ceil(geo.longitude, 5)];
  if (!_.isUndefined(geo.altitude)) {
    commandArgs.push(_.ceil(geo.altitude, 5));
  }
  await this.adbExec(commandArgs);
};
